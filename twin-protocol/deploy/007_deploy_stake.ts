...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  console.log('\n========================================');
  console.log('Deploying Stake Contract (UUPS)');
  console.log('========================================\n');
  ...
  try {
    ...
    // 获取合约工厂
    const Stake = (await ethers.getContractFactory('Stake'))as unknown as ContractFactory;
    ...
    try {
      const existingDeployment = await deployments.get('Stake');
      proxyAddress = existingDeployment.address;
      
      console.log('   Existing proxy found, importing...');
      // 使用 forceImport 导入已存在的代理
      await upgrades.forceImport(proxyAddress, Stake, { kind: 'uups' });
      stake = await ethers.getContractAt('Stake', proxyAddress);
    ...
    const tx = stake.deploymentTransaction();
    if (tx) {
    ...
    proxyAddress = await stake.getAddress();
    const implementation = await upgrades.erc1967.getImplementationAddress(proxyAddress);

    console.log('✅ Stake deployment completed!');
    console.log(`   Proxy Address: ${proxyAddress}`);
    console.log(`   Implementation Address: ${implementation}`);

    // 保存为 hardhat-deploy deployment 文件
    const artifact = await deployments.getArtifact('Stake');
      ...
      if (fs.existsSync(buildInfoDir)) {
        ...
        for (const file of files) {
          ...
          try {
            ...
            if (input && input.sources && input.sources['contracts/Stake.sol']) {
              chosen = input;
              break;
            }
          } catch {
        ...
        if (chosen) {
          ...
          const outPath = path.join(outputDir, 'Stake.json');
