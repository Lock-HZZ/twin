...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  console.log('\n========================================');
  console.log('Deploying Deposit Contract (UUPS)');
  console.log('========================================\n');
  ...
  try {
    ...
    const dustReceiver = '0x0AB1DCa339783f09cE4D8d05A1F37afbf7631C61';

    // 获取合约工厂
    const Deposit = (await ethers.getContractFactory('Deposit'))as unknown as ContractFactory;
    ...
    try {
      const existingDeployment = await deployments.get('Deposit');
      proxyAddress = existingDeployment.address;
      
      console.log('   Existing proxy found, importing...');
      // 使用 forceImport 导入已存在的代理
      await upgrades.forceImport(proxyAddress, Deposit, { kind: 'uups' });
      deposit = await ethers.getContractAt('Deposit', proxyAddress);
    ...
    console.log('✅ Deposit deployment completed!');
    console.log(`   Proxy Address: ${proxyAddress}`);
    console.log(`   Implementation Address: ${implementation}`);

    // 保存为 hardhat-deploy deployment 文件
    const artifact = await deployments.getArtifact('Deposit');

    await deployments.save('Deposit', {
      abi: artifact.abi,
      ...
      if (fs.existsSync(buildInfoDir)) {
        ...
        for (const file of files) {
          ...
          try {
            ...
            if (input && input.sources && input.sources['contracts/Deposit.sol']) {
              chosen = input;
              break;
            }
          } catch {
        ...
        if (chosen) {
          ...
          const outPath = path.join(outputDir, 'Deposit.json');
